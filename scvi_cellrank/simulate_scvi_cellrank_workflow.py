...
# Plotting helpers
# -----------------------------
def plot_absprob_corr_violin(adata: sc.AnnData, *, comparison_key: str, show: bool = True) -> None:
    ...
    ax.set_title("Cell-wise absorption-prob correlation (PCA vs scVI)")
...
def plot_terminal_mass_delta_heatmap(
    ...
    *,
    ...
    plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04, label="delta")
    if show:
        plt.show()
...
def plot_umap_instability(
    ...
    *,
    ...
    col = f"{comparison_key}:absprob_corr"
    corr = adata.obs[col].to_numpy(dtype=float)
    adata.obs["cellrank_robust:instability"] = 1.0 - corr
    adata.obs["cellrank_robust:low_stability"] = (corr < corr_threshold)

    if "X_umap" not in adata.obsm:
        sc.pp.neighbors(adata, n_neighbors=30, use_rep=umap_use_rep, random_state=0)
        sc.tl.umap(adata, random_state=0)

    sc.pl.umap(adata, color="cellrank_robust:instability", title="Instability (1 - corr)", show=show)
    sc.pl.umap(
        adata,
        color="cellrank_robust:low_stability",
        title=f"Low-stability cells (corr < {corr_threshold})",
        show=show,
    )
...
def run_workflow(
    *,
    ...
    plot_absprob_corr_violin(adata, comparison_key=comparison_key, show=True)
